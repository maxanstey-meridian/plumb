...
# .use-case.ts, .interface.ts, .handler.ts — the last added 2026-06-11 per
# scaffolder-plan D1; golden's pattern is suffix-free <module>-routes.ts).
# Sole exception: <feature>.module.ts — the Nest
...
find "$root" -type f \
  \( -name 'default-*.ts' -o -name 'base-*.ts' -o -name '*-interface.ts' \
     ...
     -o -name '*.use-case.ts' -o -name '*.interface.ts' -o -name '*.handler.ts' \) \
...
| while read -r f; do
  case "$(basename "$f")" in
    *.port.ts|*.service.ts|*.provider.ts|*.use-case.ts|*.interface.ts|*.handler.ts)

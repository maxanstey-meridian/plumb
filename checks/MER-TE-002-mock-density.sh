...
# merits review. Fork settled 2026-06-10: density is the signal, not presence —
# NSubstitute at a narrow seam is legal; this stays info, advisory only.
# DOC: testing-philosophy.md#mocks
root="$1"; [ -d "$root" ] || exit 2
rg --count-matches --no-heading -e 'Substitute\.For<' -e '\bvi\.mock\s*\(' \
  ...
  [ "$n" -gt 5 ] || continue
  printf 'MER-TE-002\tinfo\t%s:0\t%s mock constructions in one test file — review whether a hand-rolled fake would better represent the dominant collaborator\ttesting-philosophy.md#mocks\n' "${f#"$root"/}" "$n"
done
exit 0

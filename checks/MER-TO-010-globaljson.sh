...
if [ -z "$gj" ]; then
  printf 'MER-TO-010\twarn\t.:0\tno global.json — pin the SDK (.NET 10, rollForward latestFeature)\ttools.md#default-stack-1\n'
else
  rel="${gj#"$root"/}"
  grep -q '"10\.' "$gj" || printf 'MER-TO-010\twarn\t%s:0\tglobal.json does not pin the Meridian .NET 10 default\ttools.md#default-stack-1\n' "$rel"

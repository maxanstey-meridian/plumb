...
# fixtures/bad must FAIL its build with MERBE001/002/005/008/009;
...
for id in MERBE001 MERBE002 MERBE005 MERBE008 MERBE009; do
...
for expected in \
  ...
  'Application/FullyQualifiedDependency.cs.*MERBE002'; do
...
[ $fail -eq 0 ] && echo "dotnet self-test: ok (bad fails with MERBE001/002/005/008/009, good builds clean)"
exit $fail

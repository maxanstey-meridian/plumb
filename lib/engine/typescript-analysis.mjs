...
export function createTypeScriptAnalysis({ root, files, counters, diagnostic, loader = () => import("typescript") }) {
  ...
  const vueScripts = (file) => {
    ...
    if (!vueCache.has(file.path)) {
      ...
      if (!vueCompilerLoaded) {
        vueCompilerLoaded = true;
        try { vueCompiler = require("@vue/compiler-sfc"); }
        catch { diagnostic("plumb: @vue/compiler-sfc not installed under ~/.meridian/plumb — Vue rules skipped (pnpm install in plumb)"); }
      }
      if (!vueCompiler) {
        vueCache.set(file.path, Object.freeze([]));
        return vueCache.get(file.path);
      }
      const text = file.text();
      const lineMap = file.lineMap();
      const descriptor = vueCompiler.parse(text, { filename: file.path }).descriptor;
      ...
      const blocks = scripts.map((script, index) => {
        const bodyStart = script.loc.start.offset;
        const bodyEnd = script.loc.end.offset;
        const blockText = script.content;
        const language = (script.lang ?? "js").toLowerCase();

...
export function createRepositoryInventory(targetRoot, { excludes = [] } = {}) {
  const root = fs.realpathSync(path.resolve(targetRoot));
  const discovered = gitInventory(root) ?? fallbackInventory(root);
  const matcher = ignore();
  const ignoreFile = path.join(root, ".plumbignore");
  try { matcher.add(fs.readFileSync(ignoreFile, "utf8")); }
  catch (error) { if (error.code !== "ENOENT") throw error; }
  matcher.add(excludes);
  const files = [...new Set(discovered.files.map(posix))].filter((file) => !matcher.ignores(file)).sort();
